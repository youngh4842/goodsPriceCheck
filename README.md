...
`mock-shopping-mall`은 `price-monitoring-app`의 가격 조회, 가격 변경 감지, 크롤링 연동을 로컬에서 검증하기 위한 독립 Spring Boot 애플리케이션입니다. 실제 쇼핑몰이나 외부 API에 의존하지 않고, 서버 시작 시 메모리에 초기 테스트 상품을 생성합니다. 서버를 재시작하면 상품 데이터는 초기 상태로 돌아갑니다.

### 목적

- `price-monitoring-app`이 조회할 수 있는 테스트 상품 API 제공
- 상품 가격 변경 시나리오를 로컬에서 빠르게 재현
- 외부 쇼핑몰 장애나 네트워크 상태와 무관한 개발/테스트 환경 제공

### API 목록

| Method | Path | 설명 |
| --- | --- | --- |
| GET | `/api/products` | 전체 상품 목록을 조회합니다. |
| GET | `/api/products/{productCode}` | 상품 코드를 기준으로 단건 상품을 조회합니다. |
| PUT | `/api/products/{productCode}/price` | 상품 가격을 변경합니다. |
...
가격이 0보다 작은 경우 `400 Bad Request` 응답을 반환합니다. 존재하지 않는 상품 코드는 `404 Not Found` 응답을 반환합니다.

### 실행 방법

```powershell
cd mock-shopping-mall
.\gradlew.bat bootRun
```

`mock-shopping-mall`은 `8081` 포트를 사용합니다.

### price-monitoring-app과의 연동 관계

`price-monitoring-app`은 실제 쇼핑몰 대신 `mock-shopping-mall`의 상품 API를 호출해 상품 조회와 가격 변경 감지 흐름을 검증할 수 있습니다. 두 애플리케이션은 같은 Repository 안에 있지만 서로 독립적으로 빌드하고 실행할 수 있습니다.
